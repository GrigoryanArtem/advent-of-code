...
public partial class Day043(IFullInputReader input) : IPuzzleSolver
{
    ...
    private static readonly Dictionary<string, Regex> RULES = new()
    {
        ...
        { "hgt", new(@"1[5-8][0-0]cm|19[0-3]cm|59in|6[0-9]in|7[0-6]in") },
        { "hcl", new(@"#([0-9a-f]){6}") },
    ...
    {
        return _passports.Count(p => RULES.All(rf => p.ContainsKey(rf.Key))).ToString();
    }
    ...
    public string SolvePart2()
    {
        return _passports
            ...
            .Count(p => p.All(kv => RULES.ContainsKey(kv.Key) ? RULES[kv.Key].Match(kv.Value).Success : true))
            .ToString();
    }
